...
import (
	...
	_ "github.com/biya-coin/sdk-go/chain/oracle/types"
...
// SpotLeverageParams 现货杠杆系统参数
// 定义现货杠杆系统的所有配置参数，包括杠杆限制、保证金要求、风险阈值、利率模型等
type SpotLeverageParams struct {
	// max_leverage_multiplier 最大杠杆倍数（默认10倍）
	// 用户可以使用的最大的杠杆倍数，例如："10" 表示 10倍杠杆
	MaxLeverageMultiplier cosmossdk_io_math.LegacyDec `protobuf:"bytes,1,opt,name=max_leverage_multiplier,json=maxLeverageMultiplier,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"max_leverage_multiplier"`
	// single_borrow_multiplier 单次借贷倍数限制（默认3倍净值）
	// 单次借贷金额不能超过账户净值的倍数，例如："3" 表示最多借入3倍净值
	SingleBorrowMultiplier cosmossdk_io_math.LegacyDec `protobuf:"bytes,2,opt,name=single_borrow_multiplier,json=singleBorrowMultiplier,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"single_borrow_multiplier"`
	// max_absolute_borrow 单账户最大借贷额度（默认100,000 USDT）
	// 单个账户在单个币种上的最大借贷金额，例如："100000" 表示最多借入100,000 USDT
	MaxAbsoluteBorrow cosmossdk_io_math.LegacyDec `protobuf:"bytes,3,opt,name=max_absolute_borrow,json=maxAbsoluteBorrow,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"max_absolute_borrow"`
	// maintenance_margin_rate 维持保证金率（默认5%）
	// 账户需要维持的最小保证金率，例如："0.05" 表示 5%
	// 计算公式：MaintenanceMargin = TotalAssetValue × MaintenanceMarginRate
	MaintenanceMarginRate cosmossdk_io_math.LegacyDec `protobuf:"bytes,4,opt,name=maintenance_margin_rate,json=maintenanceMarginRate,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"maintenance_margin_rate"`
	// withdrawal_safety_buffer_multiplier 提现安全缓冲倍数（默认1.5倍维持保证金）
	// 提现时需要额外保留的保证金倍数，例如："1.5" 表示需要保留1.5倍维持保证金
	WithdrawalSafetyBufferMultiplier cosmossdk_io_math.LegacyDec `protobuf:"bytes,5,opt,name=withdrawal_safety_buffer_multiplier,json=withdrawalSafetyBufferMultiplier,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"withdrawal_safety_buffer_multiplier"`
	// min_margin_ratio_for_withdrawal 提现最低保证金率（默认150%）
	// 允许提现的最低保证金率，例如："1.5" 表示 150%
	MinMarginRatioForWithdrawal cosmossdk_io_math.LegacyDec `protobuf:"bytes,6,opt,name=min_margin_ratio_for_withdrawal,json=minMarginRatioForWithdrawal,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"min_margin_ratio_for_withdrawal"`
	// min_margin_ratio_for_spot_order 现货下单最低保证金率（默认120%）
	// 允许创建现货订单的最低保证金率，例如："1.2" 表示 120%
	MinMarginRatioForSpotOrder cosmossdk_io_math.LegacyDec `protobuf:"bytes,7,opt,name=min_margin_ratio_for_spot_order,json=minMarginRatioForSpotOrder,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"min_margin_ratio_for_spot_order"`
	// min_margin_ratio_for_derivative_order 期货下单最低保证金率（默认130%）
	// 允许创建期货订单的最低保证金率，例如："1.3" 表示 130%
	MinMarginRatioForDerivativeOrder cosmossdk_io_math.LegacyDec `protobuf:"bytes,8,opt,name=min_margin_ratio_for_derivative_order,json=minMarginRatioForDerivativeOrder,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"min_margin_ratio_for_derivative_order"`
	// min_margin_ratio_for_stake 质押最低保证金率（默认150%）
	// 允许质押的最低保证金率，例如："1.5" 表示 150%
	MinMarginRatioForStake cosmossdk_io_math.LegacyDec `protobuf:"bytes,9,opt,name=min_margin_ratio_for_stake,json=minMarginRatioForStake,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"min_margin_ratio_for_stake"`
	// default_min_margin_ratio 默认最低保证金率（默认120%）
	// 其他操作的最低保证金率要求，例如："1.2" 表示 120%
	DefaultMinMarginRatio cosmossdk_io_math.LegacyDec `protobuf:"bytes,10,opt,name=default_min_margin_ratio,json=defaultMinMarginRatio,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"default_min_margin_ratio"`
	// safe_risk_threshold 安全状态阈值（默认200%）
	// 保证金率超过此值时，账户处于安全状态，例如："2.0" 表示 200%
	SafeRiskThreshold cosmossdk_io_math.LegacyDec `protobuf:"bytes,11,opt,name=safe_risk_threshold,json=safeRiskThreshold,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"safe_risk_threshold"`
	// normal_risk_threshold 正常状态阈值（默认150%）
	// 保证金率在此值到 SafeRiskThreshold 之间时，账户处于正常状态，例如："1.5"
	// 表示 150%
	NormalRiskThreshold cosmossdk_io_math.LegacyDec `protobuf:"bytes,12,opt,name=normal_risk_threshold,json=normalRiskThreshold,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"normal_risk_threshold"`
	// warning_risk_threshold 警告状态阈值（默认120%）
	// 保证金率在此值到 NormalRiskThreshold 之间时，账户处于警告状态，例如："1.2"
	// 表示 120%
	WarningRiskThreshold cosmossdk_io_math.LegacyDec `protobuf:"bytes,13,opt,name=warning_risk_threshold,json=warningRiskThreshold,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"warning_risk_threshold"`
	// danger_risk_threshold 危险状态阈值（默认100%）
	// 保证金率在此值到 WarningRiskThreshold 之间时，账户处于危险状态，例如："1.0"
	// 表示 100%
	DangerRiskThreshold cosmossdk_io_math.LegacyDec `protobuf:"bytes,14,opt,name=danger_risk_threshold,json=dangerRiskThreshold,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"danger_risk_threshold"`
	// target_margin_ratio_after_liquidation 清算后目标保证金率（默认150%）
	// 清算操作执行后，账户保证金率应达到的目标值，例如："1.5" 表示 150%
	TargetMarginRatioAfterLiquidation cosmossdk_io_math.LegacyDec `protobuf:"bytes,15,opt,name=target_margin_ratio_after_liquidation,json=targetMarginRatioAfterLiquidation,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"target_margin_ratio_after_liquidation"`
	// liquidation_fee_rate 清算费率（默认1%）
	// 清算时收取的手续费率，例如："0.01" 表示 1%
	LiquidationFeeRate cosmossdk_io_math.LegacyDec `protobuf:"bytes,16,opt,name=liquidation_fee_rate,json=liquidationFeeRate,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"liquidation_fee_rate"`
	// max_liquidation_ratio 最大清算比例（默认50%）
	// 单次清算最多清算账户资产的比例，例如："0.5" 表示 50%
	MaxLiquidationRatio cosmossdk_io_math.LegacyDec `protobuf:"bytes,17,opt,name=max_liquidation_ratio,json=maxLiquidationRatio,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"max_liquidation_ratio"`
	// min_position_size 最小持仓数量阈值（默认0.00000001）
	// 持仓数量小于此值时，视为已平仓，例如："0.00000001"
	MinPositionSize cosmossdk_io_math.LegacyDec `protobuf:"bytes,18,opt,name=min_position_size,json=minPositionSize,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"min_position_size"`
	// base_interest_rate 基础利率（年化利率，默认2%）
	// 利率模型的基础利率，当利用率为 0 时的利率，例如："0.02" 表示 2% 年化利率
	BaseInterestRate cosmossdk_io_math.LegacyDec `protobuf:"bytes,19,opt,name=base_interest_rate,json=baseInterestRate,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"base_interest_rate"`
	// optimal_utilization_rate 最优利用率（默认80%）
	// 利率模型的分段点，低于此利用率时利率增长较慢，高于此利用率时利率增长较快
	// 例如："0.8" 表示 80%，当利用率超过 80% 时利率会快速上升
	OptimalUtilizationRate cosmossdk_io_math.LegacyDec `protobuf:"bytes,20,opt,name=optimal_utilization_rate,json=optimalUtilizationRate,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"optimal_utilization_rate"`
	// interest_rate_slope1 利率斜率1（默认15%）
	// 当利用率 <= OptimalUtilizationRate 时，利率随利用率增长的斜率
	// 例如："0.15" 表示 15%，利率 = BaseRate + (Utilization / OptimalUtilization)
	// × Slope1
	InterestRateSlope1 cosmossdk_io_math.LegacyDec `protobuf:"bytes,21,opt,name=interest_rate_slope1,json=interestRateSlope1,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"interest_rate_slope1"`
	// interest_rate_slope2 利率斜率2（默认100%）
	// 当利用率 > OptimalUtilizationRate 时，利率随利用率增长的斜率
	// 例如："1.0" 表示 100%，利率 = BaseRate + Slope1 + ((Utilization - Optimal)
	// / (1 - Optimal)) × Slope2
	InterestRateSlope2 cosmossdk_io_math.LegacyDec `protobuf:"bytes,22,opt,name=interest_rate_slope2,json=interestRateSlope2,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"interest_rate_slope2"`
	// max_interest_rate 最大利率上限（年化利率，默认20%）
	// 借贷利率的上限，无论利用率多高，利率都不会超过此值，例如："0.2" 表示 20%
	// 年化利率
	MaxInterestRate cosmossdk_io_math.LegacyDec `protobuf:"bytes,23,opt,name=max_interest_rate,json=maxInterestRate,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"max_interest_rate"`
	// min_supply_amount 最小供应金额（默认10）
	// 用户存入借贷池的最小金额限制，低于此值无法存入，例如："10"
	MinSupplyAmount cosmossdk_io_math.LegacyDec `protobuf:"bytes,24,opt,name=min_supply_amount,json=minSupplyAmount,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"min_supply_amount"`
	// min_borrow_amount 最小借贷金额（默认1）
	// 用户从借贷池借出的最小金额限制，低于此值无法借出，例如："1"
	MinBorrowAmount cosmossdk_io_math.LegacyDec `protobuf:"bytes,25,opt,name=min_borrow_amount,json=minBorrowAmount,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"min_borrow_amount"`
	// supplier_share_ratio 供应商利息分成比例（默认80%）
	// 借款人支付的利息中，分配给存款人的比例
	// 例如："0.80" 表示 80%，存款人获得 80% 的利息收入
	SupplierShareRatio cosmossdk_io_math.LegacyDec `protobuf:"bytes,27,opt,name=supplier_share_ratio,json=supplierShareRatio,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"supplier_share_ratio"`
	// platform_share_ratio 平台利息分成比例（默认20%）
	// 借款人支付的利息中，分配给平台的比例（进入准备金）
	// 例如："0.20" 表示 20%，平台获得 20% 的利息收入
	PlatformShareRatio cosmossdk_io_math.LegacyDec `protobuf:"bytes,28,opt,name=platform_share_ratio,json=platformShareRatio,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"platform_share_ratio"`
	// max_utilization_rate 最大利用率（默认95%）
	// 借贷池允许的最大利用率，超过此值将禁止新的借贷
	// 例如："0.95" 表示 95%，当利用率达到 95% 时禁止借贷
	MaxUtilizationRate cosmossdk_io_math.LegacyDec `protobuf:"bytes,29,opt,name=max_utilization_rate,json=maxUtilizationRate,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"max_utilization_rate"`
	// default_high_margin_ratio 默认高保证金率（无持仓时）（默认999%）
	// 当账户没有持仓时，使用的默认高保证金率，例如："999" 表示 999%
	DefaultHighMarginRatio cosmossdk_io_math.LegacyDec `protobuf:"bytes,30,opt,name=default_high_margin_ratio,json=defaultHighMarginRatio,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"default_high_margin_ratio"`
}

func (m *SpotLeverageParams) Reset()         { *m = SpotLeverageParams{} }
func (m *SpotLeverageParams) String() string { return proto.CompactTextString(m) }
func (*SpotLeverageParams) ProtoMessage()    {}
func (*SpotLeverageParams) Descriptor() ([]byte, []int) {
	return fileDescriptor_0b5851fb01a33564, []int{0}
}
func (m *SpotLeverageParams) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *SpotLeverageParams) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_SpotLeverageParams.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
...
func (m *SpotLeverageParams) XXX_Merge(src proto.Message) {
	xxx_messageInfo_SpotLeverageParams.Merge(m, src)
}
func (m *SpotLeverageParams) XXX_Size() int {
	return m.Size()
}
func (m *SpotLeverageParams) XXX_DiscardUnknown() {
	xxx_messageInfo_SpotLeverageParams.DiscardUnknown(m)
}

var xxx_messageInfo_SpotLeverageParams proto.InternalMessageInfo
...
type Params struct {
	...
	// spot_leverage_params defines the parameters for spot leverage trading
	SpotLeverageParams SpotLeverageParams `protobuf:"bytes,36,opt,name=spot_leverage_params,json=spotLeverageParams,proto3" json:"spot_leverage_params"`
}
...
func (m *Params) GetSpotLeverageParams() SpotLeverageParams {
	if m != nil {
		return m.SpotLeverageParams
	}
...
func init() {
	...
	proto.RegisterType((*SpotLeverageParams)(nil), "injective.exchange.v2.SpotLeverageParams")
...
var fileDescriptor_0b5851fb01a33564 = []byte{
	...
	,
}

func (this *SpotLeverageParams) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*SpotLeverageParams)
	if !ok {
		that2, ok := that.(SpotLeverageParams)
		if ok {
			that1 = &that2
		} else {
			return false
	...
	if !this.MaxLeverageMultiplier.Equal(that1.MaxLeverageMultiplier) {
		return false
	}
	if !this.SingleBorrowMultiplier.Equal(that1.SingleBorrowMultiplier) {
		return false
	}
	if !this.MaxAbsoluteBorrow.Equal(that1.MaxAbsoluteBorrow) {
		return false
	}
	if !this.MaintenanceMarginRate.Equal(that1.MaintenanceMarginRate) {
		return false
	}
	if !this.WithdrawalSafetyBufferMultiplier.Equal(that1.WithdrawalSafetyBufferMultiplier) {
		return false
	}
	if !this.MinMarginRatioForWithdrawal.Equal(that1.MinMarginRatioForWithdrawal) {
		return false
	}
	if !this.MinMarginRatioForSpotOrder.Equal(that1.MinMarginRatioForSpotOrder) {
		return false
	}
	if !this.MinMarginRatioForDerivativeOrder.Equal(that1.MinMarginRatioForDerivativeOrder) {
		return false
	}
	if !this.MinMarginRatioForStake.Equal(that1.MinMarginRatioForStake) {
		return false
	}
	if !this.DefaultMinMarginRatio.Equal(that1.DefaultMinMarginRatio) {
		return false
	}
	if !this.SafeRiskThreshold.Equal(that1.SafeRiskThreshold) {
		return false
	}
	if !this.NormalRiskThreshold.Equal(that1.NormalRiskThreshold) {
		return false
	}
	if !this.WarningRiskThreshold.Equal(that1.WarningRiskThreshold) {
		return false
	}
	if !this.DangerRiskThreshold.Equal(that1.DangerRiskThreshold) {
		return false
	}
	if !this.TargetMarginRatioAfterLiquidation.Equal(that1.TargetMarginRatioAfterLiquidation) {
		return false
	}
	if !this.LiquidationFeeRate.Equal(that1.LiquidationFeeRate) {
		return false
	}
	if !this.MaxLiquidationRatio.Equal(that1.MaxLiquidationRatio) {
		return false
	}
	if !this.MinPositionSize.Equal(that1.MinPositionSize) {
		return false
	}
	if !this.BaseInterestRate.Equal(that1.BaseInterestRate) {
		return false
	}
	if !this.OptimalUtilizationRate.Equal(that1.OptimalUtilizationRate) {
		return false
	}
	if !this.InterestRateSlope1.Equal(that1.InterestRateSlope1) {
		return false
	}
	if !this.InterestRateSlope2.Equal(that1.InterestRateSlope2) {
		return false
	}
	if !this.MaxInterestRate.Equal(that1.MaxInterestRate) {
		return false
	}
	if !this.MinSupplyAmount.Equal(that1.MinSupplyAmount) {
		return false
	}
	if !this.MinBorrowAmount.Equal(that1.MinBorrowAmount) {
		return false
	}
	if !this.SupplierShareRatio.Equal(that1.SupplierShareRatio) {
		return false
	}
	if !this.PlatformShareRatio.Equal(that1.PlatformShareRatio) {
		return false
	}
	if !this.MaxUtilizationRate.Equal(that1.MaxUtilizationRate) {
		return false
	}
	if !this.DefaultHighMarginRatio.Equal(that1.DefaultHighMarginRatio) {
		return false
	}
	return true
}
func (this *Params) Equal(that interface{}) bool {
	...
	if !this.SpotLeverageParams.Equal(&that1.SpotLeverageParams) {
		return false
	}
	return true
}
func (m *SpotLeverageParams) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
...
func (m *SpotLeverageParams) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *SpotLeverageParams) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	{
		size := m.DefaultHighMarginRatio.Size()
		i -= size
		if _, err := m.DefaultHighMarginRatio.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
	...
	dAtA[i] = 0xf2
	{
		size := m.MaxUtilizationRate.Size()
		i -= size
		if _, err := m.MaxUtilizationRate.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0xea
	{
		size := m.PlatformShareRatio.Size()
		i -= size
		if _, err := m.PlatformShareRatio.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0xe2
	{
		size := m.SupplierShareRatio.Size()
		i -= size
		if _, err := m.SupplierShareRatio.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0xca
	{
		size := m.MinSupplyAmount.Size()
		i -= size
		if _, err := m.MinSupplyAmount.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0xba
	{
		size := m.InterestRateSlope2.Size()
		i -= size
		if _, err := m.InterestRateSlope2.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0xb2
	{
		size := m.InterestRateSlope1.Size()
		i -= size
		if _, err := m.InterestRateSlope1.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0xaa
	{
		size := m.OptimalUtilizationRate.Size()
		i -= size
		if _, err := m.OptimalUtilizationRate.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x92
	{
		size := m.MaxLiquidationRatio.Size()
		i -= size
		if _, err := m.MaxLiquidationRatio.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x8a
	{
		size := m.LiquidationFeeRate.Size()
		i -= size
		if _, err := m.LiquidationFeeRate.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x82
	{
		size := m.TargetMarginRatioAfterLiquidation.Size()
		i -= size
		if _, err := m.TargetMarginRatioAfterLiquidation.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x7a
	{
		size := m.DangerRiskThreshold.Size()
		i -= size
		if _, err := m.DangerRiskThreshold.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x72
	{
		size := m.WarningRiskThreshold.Size()
		i -= size
		if _, err := m.WarningRiskThreshold.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x6a
	{
		size := m.NormalRiskThreshold.Size()
		i -= size
		if _, err := m.NormalRiskThreshold.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x62
	{
		size := m.SafeRiskThreshold.Size()
		i -= size
		if _, err := m.SafeRiskThreshold.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x5a
	{
		size := m.DefaultMinMarginRatio.Size()
		i -= size
		if _, err := m.DefaultMinMarginRatio.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x52
	{
		size := m.MinMarginRatioForStake.Size()
		i -= size
		if _, err := m.MinMarginRatioForStake.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x4a
	{
		size := m.MinMarginRatioForDerivativeOrder.Size()
		i -= size
		if _, err := m.MinMarginRatioForDerivativeOrder.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x42
	{
		size := m.MinMarginRatioForSpotOrder.Size()
		i -= size
		if _, err := m.MinMarginRatioForSpotOrder.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x3a
	{
		size := m.MinMarginRatioForWithdrawal.Size()
		i -= size
		if _, err := m.MinMarginRatioForWithdrawal.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x32
	{
		size := m.WithdrawalSafetyBufferMultiplier.Size()
		i -= size
		if _, err := m.WithdrawalSafetyBufferMultiplier.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x2a
	{
		size := m.MaintenanceMarginRate.Size()
		i -= size
		if _, err := m.MaintenanceMarginRate.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x22
	{
		size := m.MaxAbsoluteBorrow.Size()
		i -= size
		if _, err := m.MaxAbsoluteBorrow.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x1a
	{
		size := m.SingleBorrowMultiplier.Size()
		i -= size
		if _, err := m.SingleBorrowMultiplier.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
	...
	dAtA[i] = 0x12
	{
		size := m.MaxLeverageMultiplier.Size()
		i -= size
		if _, err := m.MaxLeverageMultiplier.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
...
func (m *Params) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	_ = l
	{
		size, err := m.SpotLeverageParams.MarshalToSizedBuffer(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *SpotLeverageParams) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	l = m.MaxLeverageMultiplier.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.SingleBorrowMultiplier.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.MaxAbsoluteBorrow.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.MaintenanceMarginRate.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.WithdrawalSafetyBufferMultiplier.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.MinMarginRatioForWithdrawal.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.MinMarginRatioForSpotOrder.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.MinMarginRatioForDerivativeOrder.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.MinMarginRatioForStake.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.DefaultMinMarginRatio.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.SafeRiskThreshold.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.NormalRiskThreshold.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.WarningRiskThreshold.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.DangerRiskThreshold.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.TargetMarginRatioAfterLiquidation.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.LiquidationFeeRate.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.MaxLiquidationRatio.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.MinPositionSize.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.BaseInterestRate.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.OptimalUtilizationRate.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.InterestRateSlope1.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.InterestRateSlope2.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.MaxInterestRate.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.MinSupplyAmount.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.MinBorrowAmount.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.SupplierShareRatio.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.PlatformShareRatio.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.MaxUtilizationRate.Size()
	n += 2 + l + sovExchange(uint64(l))
	l = m.DefaultHighMarginRatio.Size()
	n += 2 + l + sovExchange(uint64(l))
	return n
}
...
func (m *SpotLeverageParams) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		if wireType == 4 {
			return fmt.Errorf("proto: SpotLeverageParams: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: SpotLeverageParams: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxLeverageMultiplier", wireType)
			}
			...
			if err := m.MaxLeverageMultiplier.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SingleBorrowMultiplier", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.SingleBorrowMultiplier.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxAbsoluteBorrow", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MaxAbsoluteBorrow.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaintenanceMarginRate", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MaintenanceMarginRate.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field WithdrawalSafetyBufferMultiplier", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.WithdrawalSafetyBufferMultiplier.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MinMarginRatioForWithdrawal", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MinMarginRatioForWithdrawal.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 7:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MinMarginRatioForSpotOrder", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MinMarginRatioForSpotOrder.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 8:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MinMarginRatioForDerivativeOrder", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MinMarginRatioForDerivativeOrder.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 9:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MinMarginRatioForStake", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MinMarginRatioForStake.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 10:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DefaultMinMarginRatio", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.DefaultMinMarginRatio.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 11:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SafeRiskThreshold", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.SafeRiskThreshold.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 12:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field NormalRiskThreshold", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.NormalRiskThreshold.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 13:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field WarningRiskThreshold", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.WarningRiskThreshold.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 14:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DangerRiskThreshold", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.DangerRiskThreshold.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 15:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TargetMarginRatioAfterLiquidation", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.TargetMarginRatioAfterLiquidation.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 16:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field LiquidationFeeRate", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.LiquidationFeeRate.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 17:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxLiquidationRatio", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MaxLiquidationRatio.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 18:
		...
		case 20:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field OptimalUtilizationRate", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.OptimalUtilizationRate.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 21:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field InterestRateSlope1", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.InterestRateSlope1.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 22:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field InterestRateSlope2", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.InterestRateSlope2.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 23:
		...
		case 24:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MinSupplyAmount", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MinSupplyAmount.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 25:
		...
		case 27:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SupplierShareRatio", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.SupplierShareRatio.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 28:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PlatformShareRatio", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.PlatformShareRatio.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 29:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxUtilizationRate", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MaxUtilizationRate.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 30:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DefaultHighMarginRatio", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.DefaultHighMarginRatio.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
...
func (m *Params) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 36:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SpotLeverageParams", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.SpotLeverageParams.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
