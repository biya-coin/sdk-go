...
func main() {
	if len(os.Args) < 3 {
		...
		fmt.Printf("示例: go run example.go %s 1.5  （卖单价格应低于买单价格 %s 才能撮合）\n", sellPrice, buyPrice)
		return
	}
	...
	orderId := fmt.Sprintf("sell_%d", time.Now().UnixNano())
	fmt.Printf("\n下卖单: market=%s..., price=%s, quantity=%s, cid=%s\n",
	...
	fmt.Printf("✓ 卖单已发送到 mempool\n")

	// 等待上链
	fmt.Println("等待交易确认...")
	time.Sleep(3 * time.Second)

	// 查询订单（撮合后可能已成交，挂单数可能为0）
	...
	if !found {
		fmt.Printf("★ 卖单 %s 未在挂单列表中 → 可能已被撮合成交\n", orderId)
	}
}
